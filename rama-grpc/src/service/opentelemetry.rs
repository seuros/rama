...
/// Wrapper type which allows you to use a rama gRPC [`Service`]
...
impl<S> OtelExporter<S> {
    ...
    fn apply_env(&mut self) -> Result<(), OtelExporterConfigError> {
        ...
        if let Some(compression) = env_compression(OTEL_EXPORTER_OTLP_COMPRESSION)? {
        ...
        if let Some(compression) = env_compression(OTEL_EXPORTER_OTLP_TRACES_COMPRESSION)? {
        ...
        if let Some(compression) = env_compression(OTEL_EXPORTER_OTLP_METRICS_COMPRESSION)? {
            ...
            runtime.block_on(async move {
                ...
                let resource_spans = {
                    let guard = resource.read().map_err(|_| {
                        OTelSdkError::InternalFailure("resource lock poisoned".to_string())
                    })?;
...
fn env_compression(
    var: &'static str,
) -> Result<Option<Option<CompressionEncoding>>, OtelExporterConfigError> {
    let value = match std::env::var(var) {
        Ok(value) => value,
        Err(std::env::VarError::NotPresent) => return Ok(None),
        Err(err) => {
